...
exports.default = (config) => {
    const { NODE_ENV: { dev, prod }, useEslint, useReact, useVue, useTypeScript } = config;
    const __ISDEV__ = dev.includes(process.env.NODE_ENV);
    const __PROD__ = prod.includes(process.env.NODE_ENV);
    const useStyleLoaders = (loader, loaderOptions) => {
        return [
            __ISDEV__ ? 'style-loader' : mini_css_extract_plugin_1.default.loader,
